tss <- function(x, u, v, m, t=NULL, eta, tidx="f"){
  if(missing(x))
  ...
  minv <- Inf
  totdiffv <- as.double(0)
  for(i in 1:(k-1))
    for(j in (i+1):k)
      totdiffV = totdiffv + sum((V[i,]-V[j,])^2)
      if(sum((V[i,]-V[j,])^2) < minv) 
        minv <- sum((V[i,]-V[j,])^2)
  if(tidx == "e")
    idx <- (sum(((U)^m+(T)^eta)*D)+totdiffv/(k*(k-1)))/(minv + 1/k) 
  else
    idx <- (sum((U^m)*D)+totdiffv/(k*(k-1)))/(minv + 1/k) 
  if(tidx == "f")
    names(idx) <- "tss"
  else if(tidx == "e")
    names(idx) <- "tss.e"
