cs <- function(x, u, v, m, t=NULL, eta, tidx="f"){
  if(missing(x))
  ...
  if(tidx == "g"){
    if(!is.null(T))
      ...
      stop("Typicality matrix is required to compute the generalized CS index")
  }
  n <- nrow(U)
  k <- ncol(U)
  if(dcheck){
  ...
  for(j in 1:k){
    for(i in 1:n) 
      totdifv <- 0
      for(l in 1:k) 
        totdifv <- totdifv + sum((V[j,]-V[l,])^2)
    if(tidx == "e")
      cs <- cs + sum(((U)^m + (T)^eta)* D)/sum(U + T) * totdifv 
    else
      cs <- cs + sum((U)^m * D)/sum(U) * totdifv 
  }
  idx <- cs 
  if(tidx == "f")
    names(idx) <- "cs"
