kwon <- function(x, u, v, m, t=NULL, eta, tidx="f"){
  if(missing(x))
  ...
  if(tidx == "g"){
    if(!is.null(T))
      ...
      stop("Typicality matrix is required to compute the generalized kwon index")
  }
  n <- nrow(U)
  k <- ncol(U)
  if(dcheck){
  ...
  if(tidx == "e")
    idx <- (sum(((U)^m + (T)^eta)* D) + sum(vdiff)/k)/minv
  else
    idx <- (sum((U)^m * D) + sum(vdiff)/k)/minv
  if(tidx == "f")
    names(idx) <- "kwon"
  else if(tidx == "e")
    names(idx) <- "kwon.e"
